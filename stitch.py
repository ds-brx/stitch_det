...
def stitch_by_H(img1, img2, H, frame_number):
	...
	# Get the coordinate range of output (0.5 is fixed for image completeness)  ##what is the meaning of image completeness
	...
	# The output matrix after affine transformation  ##why?
	...
	#print(frame_number)
	if frame_number == 1:  #what is ramp
...
def stitch_image(img1, img2, frame_number, temp=None,i=None):
	...
	img2 = cylindrical_project(img2,temp)     ##same cylindrical projection to both images? Why?
	...
	if(i==1):  ##why is homography constant?
		H = np.load('i1i2.npy')
...
def crop_black(img):
	...
	# 	best_rect = (3,28,1134,490)
		...
		img_gray = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY) ## GPU cvtcolor function
		_, thresh = cv2.threshold(img_gray, 1, 255, cv2.THRESH_BINARY)  ## GPU threshold function
		contours, _ = cv2.findContours(thresh, cv2.RETR_EXTERNAL,  ## no gpu function// check profile for time taken
										  cv2.CHAIN_APPROX_NONE)

		max_area = 0
		best_rect = (0, 0, 0, 0)

		for cnt in contours: ## make threads?
			x, y, w, h = cv2.boundingRect(cnt) ## no gpu function// check profile for time taken
			deltaHeight = h - y
			deltaWidth = w - x
			...
			if area > max_area and deltaHeight > 0 and deltaWidth > 0:
				max_area = area
				best_rect = (x, y, w, h)    ## best reactangle on the basis of counter points generated? How?
...
def cylindrical_project(img,temp = None):
	...
	if(temp is None):
		pass
		# h_,w_ = img.shape[:2]
		# K = np.array([[600,0,w_/2],[0,600,h_/2],[0,0,1]])  #dk

		# # pixel coordinates
		# y_i, x_i = np.indices((h_,w_))  #dk
		# X = np.stack([x_i,y_i,np.ones_like(x_i)],axis=-1).reshape(h_*w_,3) #dk 
